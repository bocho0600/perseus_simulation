...
| `depth2/bucket.urdf` | the same export re-run at `cad: max_depth: 2` — the source for the bucket's FRONT/REAR split |
| `depth2/export.py` | the script that produced it (run it from a scratch directory) |

The STL meshes from `output/meshes/` are **not** duplicated here — they were
...
- **The bucket is a clamshell.** `Bucket_1` contains "Bucket FRONT Frame",
  ...
  to. At `cad.max_depth: 0` all of that fuses into one rigid body; at depth 1 the
  two halves separate, and at depth 2 the actuators split into barrel and shaft.
  The jaw hinge is `Revolute_1-2`, axis `0 1 0`.
- **The jaw actuators are NOT skewed, despite appearances.** The two rod mate
  connectors are 447 mm apart and the two base connectors 393 mm, which reads as
  a 27 mm sideways lean per actuator. It is an artefact: a revolute mate's
  rotation axis is its connector's local z, which for these pins runs laterally,
  so the origin can sit anywhere along the pin. Drop that coordinate and both
  actuators are identical and planar (base `y,z = 0.012, -0.032`, rod
  `-0.19474, -0.02634` in the REAR frame; 206.82 mm pin to pin). The same trap
  applies to any mate here — compare connectors only in the plane normal to the
  axis. The slider limits are the toolkit's default `-0.25 … 0`, not real stroke
  limits.
...
2. **Set real slider limits on the jaw actuators.** The description's jaw travel
   (`-0.8489 … 0`) follows from the 50 mm stroke, the hinge geometry and the
   confirmed fact that the CAD is drawn with the actuators fully extended; real
   mate limits on the slider mates would replace that derivation.
