...
That gives three independent degrees of freedom, one per actuator group the real
robot drives over CAN (`bucket_driver` in the `payloads` package writes
`LIFT_BOTH`, `TILT_BOTH` and `JAWS_BOTH`), and each has an encoder at both ends:

- **lift** — the arms pitch, raising and lowering the bucket fixture
- **tilt** — the bucket pitches about the arm tips
- **jaws** — the bucket is a **clamshell**: a FRONT half (the two teal side plates
  and the tray) hinges off a REAR half at the plate apexes, driven by two 50 mm
  actuators

Six encoder positions in total: lift at the gantry bearing blocks, tilt at the
bearing blocks where the arms meet the bucket, and jaws at the plate apexes.
    ...
    └─ bucket_mounting_frame          ← interface link, base_link axes
       └─ bucket_frame_joint (fixed)
          └─ bucket_frame             ← gantry, bearing blocks, rails
             ...
             │     │  └─ bucket       ← REAR half: back plate, bearing blocks, clip mount
             │     │     └─ bucket_jaw_joint (revolute) ▸ DRIVEN
             │     │        └─ bucket_jaw  ← FRONT half: side plates and tray
...
| `bucket_jaw_joint` | revolute, axis `0 1 0` | −0.8489 … 0 | 91.4 N·m | 1.64 rad/s | the CAD pose, actuators fully extended (confirmed) |

Positive lift raises. Positive tilt curls the scoop back to retain a load;
negative dumps. Positive jaw closes (the tray swings back toward the REAR half);
negative opens.
...
**Jaws.** The FRONT half hinges at the plate apexes — the CAD's `Revolute_1-2`,
axis `0 1 0`, at (+0.0765, +0.3485, +0.1589) from the tilt pivot. The 50 mm
actuator's rod pin is only 63 mm from that hinge with the base pin 202 mm below
it, so `dL/dθ` at the CAD pose is 0.0609 m/rad and the 50 mm stroke sweeps
0.82 rad (47°).

The two rod connectors are 447 mm apart and the two base connectors 393 mm, but
that is **not** a 27 mm skew — see §5. Both actuators are planar and identical.

> **Which end of the jaw stroke is the CAD pose?** Neither the export nor the mate
> limits say (the toolkit fills every slider with a default `-0.25 … 0`), so this
> was confirmed against the real machine: the CAD is drawn with the actuators fully
> **extended** — the FRONT frame is already tight against the REAR frame and cannot
> be pushed any closer — so the jaws can only open from there: `−0.8489 … 0`.
...
| `bucket` (REAR half) | 3.389 kg | back plate, clip mount, 4 bearing blocks and 10 bearings, the two jaw-actuator barrels + half the tilt ram |
| `bucket_jaw` (FRONT half) | 0.678 kg | the FRONT frame + the two jaw-actuator shafts |
...
the chassis), so `bucket_arm`, `bucket` and `bucket_jaw` are the moving bodies.
...
- **the scoop is four boxes** — the back plate on `bucket`, and the floor and the
  two side structures on `bucket_jaw` (so they swing with the jaws), fitted to the
  measured plate positions. Keeping them separate is what makes
...
All five rams — two lift, one tilt and the two 50 mm jaw actuators — are
animated the same way and by the same follower node.

The jaw pair looked for a while like it needed a 3-D follower, because the two
base mate connectors are 393 mm apart and the two rod connectors 447 mm, which
reads as a 27 mm sideways skew per actuator. It is not. A revolute mate's
rotation axis is its connector's local z, and for these pins that axis points
along the lateral direction, so the connector origin slides freely along the pin
without moving it. Ignore that coordinate and both actuators have identical
in-plane geometry, so an ordinary planar pitch joint aims them exactly.
...
`bucket_driver` reads `LIFT_BOTH`, `TILT_BOTH` and `JAWS_BOTH`: one encoder at
each **end** of each pivot axis, six in all. The URDF has one joint per axis, and
a joint sits at a single point along its axis, so the other end had no frame.
Six massless fixed frames, on the fixed side of each joint and exactly on its
axis, mark them ("left" is +y):

| Frame pair | Parent | Ends | Separation |
|---|---|---|---|
| `bucket_lift_encoder_left/right` | `bucket_frame` | the two arm pivots | 0.200 m |
| `bucket_tilt_encoder_left/right` | `bucket_arm` | the two arm tips | 0.202 m |
| `bucket_jaw_encoder_left/right` | `bucket` | the two hinge bosses on the FRONT frame | 0.453 m |

The jaw spacing is measured from the FRONT frame mesh: its hinge bosses sit at
0 and -0.453 m along the axis. Confirmed in a running sim through TF. They change
nothing physically, and `use_bucket:=false` is still 33 links.
...
### Viewing it without Gazebo

```bash
ros2 launch perseus_description view_perseus.launch.py payload:=bucket
```

Starts `robot_state_publisher`, RViz and a slider GUI with the bucket attached.
`gui:=false` drops RViz and the sliders and leaves only the TF tree, for SSH.
`payload:=none` (the default) is the bare rover. `payload` is defined here; the
perseus-v3 `description` package does not have it.

RViz is launched as plain `rviz2`. This workspace is pixi-managed and pixi's
`rviz2` is a conda build, which dies with `Invalid parentWindowHandle` if wrapped
in nixGL. `use_nixgl:=true` restores the wrapper for a nix-provided `rviz2`.

The rams are not free joints: each is a function of the lift, tilt and jaw angles,
and with no Gazebo nothing would drive them. So with `payload:=bucket` the slider
GUI is rerouted through `bucket_ram_follower` in `viewer_mode`:

- the GUI publishes `/joint_states_raw`; the node overwrites the ten ram joints
  with the exact values and republishes `/joint_states`;
- it also republishes the description as `/robot_description_sliders` with the ram
  joints set to `fixed`, and the GUI reads that, so the panel shows only the four
  wheels plus lift, tilt and jaw. (Fixed joints get no slider. The slider node's
  `dependent_joints` parameter looked like the obvious way to hide them, but it is
  overwritten by the URDF's mimic tags as soon as the description arrives.)

`robot_state_publisher` still reads the real description, so the rams move.
...
ros2 topic pub /bucket/jaw/cmd_pos  std_msgs/msg/Float64 "{data: -0.4}"   # open the jaws
```
...
| `/bucket/jaw/cmd_pos` | `std_msgs/Float64` | → sim |
...
| `/joint_states` | `sensor_msgs/JointState` | sim → ROS (all 9 bucket joints) |
...
| `jaw_lower` / `jaw_upper` | −0.8489 / 0 | you have the real jaw actuator stroke or limits (the range follows from the 50 mm stroke and the hinge geometry) |
